<template>
  	<section>
    	<header class="top_tips">
    		<span class="num_tip" v-if="fatherName == 'home'">{{level}}</span>
    		<span class="num_tip" v-if="fatherName == 'item'">题目{{itemNum}}</span>
    	</header>
    	<div v-if="fatherName == 'home'" >
    		<div class="home_logo item_container_style"></div>
    		<router-link to="item" class="start button_style" ></router-link>
    	</div>
    	<div v-if="fatherName == 'item'" >
...
import src from './../images/1-1.jpg'
export default {
	name: 'itemcontainer',
	data() {
		return {
...
//		window.document.body.style.backgroundImage = 'url(./static/img/1-1.jpg)';
	},
	beforeMount:()=>document.body.style.backgroundImage=src
}

</script>

<style lang="less" >
	body{background-image: url(../images/1-1.jpg);}
	.top_tips{
		position: absolute;
		height: 7.35rem;
		width: 3.25rem;
